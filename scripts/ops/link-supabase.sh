...
: "${SUPABASE_ACCESS_TOKEN:?Set SUPABASE_ACCESS_TOKEN in .env.local (Supabase personal access token)}"
: "${SUPABASE_PROJECT_ID:?Set SUPABASE_PROJECT_ID in .env.local (project ref from Dashboard → Settings → General)}"
: "${SUPABASE_DB_PASSWORD:?Set SUPABASE_DB_PASSWORD in .env.local (database password)}"
